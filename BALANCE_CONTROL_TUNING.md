...
第四题的位置模式对照控制器及其独立调参入口见
`TASK4_POSITION_CONTROL.md`。
...
无法发送速度命令。速度/停止/位置动作命令发送后保留 2 ms 总线换向时间，
避免驱动器的四字节动作确认帧与紧随其后的 `0x36` 请求碰撞；接收器也会丢弃
混入的 `F6/FD/FE/F3` 确认帧并重新对齐真正的位置回包。
...
6. 单次或连续少量位置丢包时沿用最近一次有效角度；超过 120 ms 没有任何
   新位置才停止电机，不允许长期无角度反馈运行。
...
Emm_V5.0 标准 `0x36` 完整回包为 8 字节（不计地址为 7 字节）：
`地址 + 0x36 + 符号 + 4 字节位置 + 0x6B`。底层也兼容已有的 6 字节
`地址 + int32 位置 + 0x6B` 格式，两种格式都归一化为带符号原始位置计数，
角度换算为：
...
| `motor_position_timeout_ms` | 120 ms | 短时丢包保持末值，连续超时后停机 |
...
9. 最后验证视觉 `none` 时杆能回水平，拔掉位置反馈时电机能在 120 ms 左右
   停止，再逐步放开速度和角度限制。
