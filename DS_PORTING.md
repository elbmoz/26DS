...
- 第二题当前只持续调节到中心，不自动结束；再次按按键 2 会立即停止 3 号
  电机、停止位置查询并发送 `ok`。机械零点、传动比、角度限幅和两级控制
  参数集中在 `balance_control_config`，后续标定后再接入
  `+5 cm -> -5 cm` 分段状态机。
- 选择题号 9 后按按键 2，进入独立的 3 号电机位置与 IMU 数据采集页。
  监视器每 100 ms 发送 `03 36 6B`，按协议显示原始位置 `P` 和换算角度。
  ...
  运动为 `+上幅度`；检测到电机位置确实变化、随后连续 3 次位置更新稳定，
  并在端点停留 500 ms 后，才用 `上幅度+下幅度` 反向跨越两个端点。禁止按
  固定周期提前反向；单次运动超过 5 秒仍未稳定会停机并报告超时。USART6
  同时以 5 Hz 保留发送 `Q9,...\n` 遥测，包含电机位置、相对三轴角、有效
  状态和最近运动状态。该任务不启动第一题或第二题控制器，也不修改
...
6. 运行题目 9 采集多个完整上下端循环并保存 `Q9` 遥测，同时记录本次使用的
   ...
   和稳定判据。题目 9 不自动标定或覆盖题目 2 参数；上传实测数据并完成
   离线分析后，再填写机械零点、`rod_angle_per_motor_degree`、方向和控制
   参数。
