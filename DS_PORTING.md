...
13. `MotorPositionMonitor.c/.h`：第二题/第九题共享的 3 号电机非阻塞位置读取。
...
0.01°；`motor_command` 仍是最终 RS485 速度命令。手动停止时 STM32 等待
...
- 选择题号 2 后按按键 2，启动 USART6 视觉、非阻塞 `0x36` 电机位置读取和
  ...
  内环根据实际杆角产生速度命令。视觉失效时杆回到水平；电机位置失效时
  立即停机。
...
- 题目 3~8 暂时显示 `NOT READY`；按按键 2 返回选题菜单。
