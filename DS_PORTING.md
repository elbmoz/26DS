...
13. `MotorPositionMonitor.c/.h`：第九题只读查询 3 号电机实时位置。
...
- `DS_Init()` 完成后、进入菜单前，当前 `main.c` 会向 3 号电机发送一次
  `+240` 脉冲、速度 50、加速度 10 的相对位置命令。它不是回零，复位会
  再次执行；上车前必须确认机械行程。
- 上电进入选题菜单，题号初始显示为 0。
...
- 选择题号 9 后按按键 2，进入独立的 3 号电机位置监视页。监视器每 100 ms
  发送 `03 36 6B`，按协议把 6 字节回包中的 4 字节大端 `int32_t` 显示为
  原始位置 `P`，并按 `位置 * 360 / 65536` 显示角度 `A`。该任务不启动
  第一题循迹或第二题 PID；再次按按键 2 停止查询。底层也兼容带 `0x36`
  和独立符号字节的 8 字节回包，该格式的角度按 `位置 / 10` 显示。
  监视期间 3 号电机每隔 1 秒交替执行 `+50`、`-50` 脉冲相对运动，速度
  50、加速度 10；退出任务 9 时立即停止 3 号电机。正常页面同时显示
  JY61P 的 `GX/GY/GZ` 三轴角速度；通信失败时切换到电机原始回包诊断页。
...
| 新增 | `MotorPositionMonitor` | 第九题周期读取并显示 3 号电机位置 |
