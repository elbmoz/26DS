...
| OLED | PB8=SCL、PB9=SDA，软件 I2C，开漏输出 |

电机地址、方向和红外引脚集中在 `Core/Inc/DS.h`。按键映射在
`Core/Inc/button.h`，OLED 映射在 `Core/Inc/OLED.h`。接线改变时应同步修改
头文件、`Core/Src/gpio.c` 和 `gc.ioc`。
...
第一题运行时 OLED 显示运行时间、8 路红外状态、横向误差和转向修正量。
目前没有明确的终点线判定规则，所以使用按键 2 手动结束。后续确定终点线
特征后，应把自动停止条件加入 `DS_Task_Run()`。

## OLED 移植结果

拖入的 OLED 驱动原本是 STM32F103 标准外设库版本，现已改成 STM32F407 HAL
软件 I2C，并加入 Keil 工程。默认使用常见 SSD1306 地址 `0x3C`
（写地址字节 `0x78`）。

软件 I2C 现在使用 DWT 保证 SCL/SDA 跳变间隔，速率约为 200 kHz，并读取每个
字节后的 ACK。初始化会先恢复可能卡住的总线，然后自动探测 `0x3C` 和 `0x3D`。
可在 Keil Watch 中直接观察 `ds_task.oled_ready`：1 表示收到 ACK，0 表示
初始化通信失败。`ds_task.oled_address` 显示识别到的 7 位地址（`0x3C` 或
`0x3D`）。上电成功通信后会先全屏点亮 200 ms，再显示任务菜单。
...
| 复用并移植 | `OLED` | 从 F103 标准库改为 F407 HAL 软件 I2C |
